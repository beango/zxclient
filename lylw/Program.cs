...
namespace ZXClient
{
    static class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                Exception ex = Utils.Utils.KillProcess("adb");
                Tools.ShowInfo2("销毁进程adb" + ex.Message);

                //ex = Utils.Utils.KillProcess("ZXEval");
                //Tools.ShowInfo2("销毁进程ZXEval" + ex.Message);

            }
            catch (Exception ex)
            {
                Tools.ShowInfo2("销毁进程" + ex.Message);
            }
            try
            {
