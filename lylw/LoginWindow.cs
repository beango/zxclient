    ...
    public partial class LoginWindow : Form
    {
        ...
        internal static string UserLogin(string card, string pw, int isMember)
        {
            ...
            if (ReturnDatastr == MainData.loginSuccess)
            {
                ...
                HttpUtil.GetData("http://localhost:9107/Login?UserID=" + card, (sender, e) =>
                        {
                            if (e.Error != null)
                            {
                                Tools.ShowInfo2("登录结果:" + e.Error.Message);
                                return;
                            }
                            String rst = e.Result; //.GetString(Encoding.UTF8);
                            LogHelper.WriteInfo(typeof(HttpUtil), "登录请求结果：" + rst);
                        });
